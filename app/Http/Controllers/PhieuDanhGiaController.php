...
class PhieuDanhGiaController extends Controller
{
    ...
    public function luuphieudanhgia(Request $request)
    {
        ...
        if (PhieuDanhGia::where('ma_phieu_danh_gia', $ma_phieu_danh_gia)->exists()) {
            ...
            $mau01A = Mau01A::all();
            foreach ($mau01A as $mau01A) {
                $ket_qua_mucA = new KetQuaMucA();
                $ket_qua_mucA->ma_phieu_danh_gia = $ma_phieu_danh_gia;
                $ket_qua_mucA->ma_tieu_chi = $mau01A->ma_tieu_chi;
                $ket_qua_mucA->diem_toi_da = $mau01A->diem_toi_da;
                $temp = $mau01A->ma_tieu_chi;
                $ket_qua_mucA->diem_tu_cham = $request->temp;
                $ket_qua_mucA->save();
            }            

            //Lưu kết quả tự chấm Mục B
            $ket_qua_mucB = new KetQuaMucB();
